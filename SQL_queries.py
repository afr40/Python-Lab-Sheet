...
def create_table_flight(result):
    cursor.execute("SELECT * FROM FlightInfo LIMIT 0")
...
destination_query = "SELECT * FROM FlightInfo WHERE Destination = 'Madrid-MAD'"
...
status_query = "SELECT * FROM FlightInfo WHERE Status = 'Delayed'"
cursor.execute(status_query)
status_results = cursor.fetchall()
create_table_flight(status_results)

print("Flight Retrival: Flights Between February and April")
months_query = "SELECT * FROM FlightInfo WHERE Departure_date > '2025-02-01' AND Departure_date < '2025-04-30'"
...
time_query = "UPDATE FlightInfo SET Schedule_time = '11:15' WHERE FlightID = 1"
cursor.execute(time_query)
status_query = "UPDATE FlightInfo SET Status = 'Delayed' WHERE FlightID = 1"
cursor.execute(status_query)
updated_query = "SELECT * FROM FlightInfo WHERE FlightID = 1"
...
assigment_query = "UPDATE FlightInfo SET PilotID = 4 WHERE FlightID = 1"
cursor.execute(assigment_query)
cursor.execute("SELECT * FROM FlightInfo WHERE FlightID = 1")
...
print("\n")
view_query = "SELECT FlightID, Origin, Destination FROM FlightInfo WHERE FlightID = 4"
cursor.execute(view_query)
view_results = cursor.fetchall()
print(pd.DataFrame(view_results, columns=['FlightID', 'Origin', 'Destination']))
print("\n")

update_destination = "UPDATE FlightInfo SET Destination = 'Singapore' WHERE FlightID = 4"
...
query = '''
    SELECT Name, FlightInfo.PilotID, COUNT(*)
    FROM FlightInfo JOIN Pilot 
    WHERE FlightInfo.PilotID = Pilot.PilotID 
    GROUP BY FlightInfo.PilotID HAVING COUNT(*) > 1
    ORDER BY COUNT(*) DESC
    '''
